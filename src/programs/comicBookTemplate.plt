...
fn comicBookTemplate(list num rowDetails, list num colDetails):
    num maxHeight
    num maxLength
    num x
    num y
    ...
    y=0
    j = 0
    for i=0;i<rowDetails.length();i=i+2:
        x = 0
        
        #Border for the row
        h = maxHeight * rowDetails[ i ]
        rectFill((0,y), h, maxLength,"brown") 
        rectFill( ( 5 , y+5), h - 10, maxLength - 10, "white" )
        
        #Putting the columns
        for k=0; k +1 < rowDetails[i + 1];k=k+1:
            x = x + colDetails[j+k]*maxLength
            rectFill(( x , y), h , 5 , "black" ) 
            #printXY("*",( x - 20 , y+h/2)) 
        end
        j = j + k + 1
        y = y+h - 5
...
list num rd
list num cd
#Give row height % and no of columns in that row
rd = [0.3, 2, 0.4, 1, 0.3, 2]

#Give column % wrt the whole width
cd = [0.3,0.7,1,0.7,0.3]

comicBookTemplate(rd, cd)
